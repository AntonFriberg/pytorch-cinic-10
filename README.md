...
| Model                                                 | Accuracy | Native Training Time | Docker Training Time | Kubernetes Training Time |
| ----------------------------------------------------- | -------- | -------------------- | -------------------- | ------------------------ |
| [VGG16](https://arxiv.org/abs/1409.1556)              | 84.74%   | 3 hours 12 minutes   | 3 hours 40 minutes   | 3 hours 21 minutes       |
| [ResNet18](https://arxiv.org/abs/1512.03385)          | 87.45%   | 4 hours 41 minutes   | 4 hours 24 minutes   | 7 hours 25 minutes*      |
...
| Software      | Native Version | Docker/k8s Version |
| ------------- | -------------- | ------------------ |
...
\* These results where run on another kubernetes node running the same graphics
card model. The reason these were slower were most likely due to running four of
them in parallel causing them to limit clock-speeds due to heat limitations.
Note that the experiments was always run utilizing only a single card but, in
the kubernetes case, multiple experiments was being run simultaneously.
